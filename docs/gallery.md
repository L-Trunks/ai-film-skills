...
## 《镜像》第一章·雨夜钟楼 —— long-shot 路线，交付物从「整片」改成「精片段」

<img src="assets/jingxiang-zhonglou.gif" width="400">

▶ **[钟楼下（33 秒，有声）](assets/jingxiang-zhonglou.mp4)** —— 9 镜 8 句台词，双女主正反打

<img src="assets/jingxiang-tiantai.gif" width="400">

▶ **[天台（14 秒，有声）](assets/jingxiang-tiantai.mp4)**

| | |
|---|---|
| profile | `minimax-h3-local` |
| 结构 | `episode-drama`，日漫画风，双女主对白戏 |
| 生成分辨率 | 736×416，5.17 秒/段 |
| 全片生成量 | **58 段 / 3 小时 21 分，失败 0** |
| 实际交付 | **不是那部 5 分钟的全片**，而是从中挑出的两段 33 秒 / 14 秒 |
| 声音 | IndexTTS-2.5 情绪向量配音（人耳选版），BGM 分段 |

《狱卒》那条线的结论是「12 GB 卡上做得出片子，做不出短剧」。
这部片是那句话的下一步：**不跟这个限制硬碰，改交付目标。**
全片照跑（它便宜，一夜就出来了），但**不当成品交** —— 它变成了挑片段用的镜池。

### ① 选片段的判据是景别，不是剧情

这条是用户判的，而且和直觉相反 —— 我第一版是按「哪几镜剧情最完整」挑的，被毙。

| 景别 | 在 0.31 MP 上 | 用不用 |
|---|---|---|
| 远景定场 | 人小，脸糊成一团 | ⛔ 一段 30 秒可以**零远景** |
| 全景双人 | 同上 | ⛔ |
| 过肩 / 近景 | 脸够大，质量最好 | ✅ 主力 |

远景本来是用来交代「这是哪儿、有谁」的。**那个信息用台词和声音补，比放一个糊掉的定场镜强。**

### ② 片段是重剪的，不是从全片时间轴上切一段下来

第一版就是拿剪刀在全片上划了一段，结果**片段里连着三个同一机位的镜**，
读起来像「一张图在动嘴」—— 因为全片里跟它们配对的那些反打镜，切下来之后全在片段外面。

修法是按片段自己的镜头表重排：从全片的镜池里挑，把别的角色、别的机位的反应镜
重新插回去，让正反打**在片段内部重新成立**。

### ③ 第一版整档废，病因是没有角色参考图

这部片的第一版被用户判「人物一致性崩」，而且**是用户判的，我自己没查出来**。

病因不是画风：z-image 纯文生图出 47 张关键帧，**等于 47 次独立抽卡**，
中间没有任何东西把「这是同一个人」钉住；再加上两个角色的锚点强度不对等，
弱的那个一路漂。

第二版换日漫画风，每个角色锚三样：**发型形状 + 高饱和虹膜 + 一个不可能漂的硬记号**。
实拍零串装。⭐ 附带一条：**治不过的先验就顺着它写** ——
模型死活要给这个角色穿藏蓝制服，那就把藏蓝制服写进造型圣经。

⚠️ 原创角色没有 cos 角色那种「名字自带形象」的先验，必须自己造等价强度的锚。

### 它也是两份新文档的出处

- `knowledge/grid-review.md`（格子法）—— 一天之内在关键帧定稿图、关键帧候选图、
  视频段三处各栽一次「用『跑完了没报错』代替『好不好』」。
- `pitfalls.md` 坑 ⑲（中间目录静默复用过期数据）—— 两天的配音工作一条都没进成片，
  **而且我连着两轮给出假绿灯**，因为我量的是 `vo\`、成片读的是 `vo_剪\`。
