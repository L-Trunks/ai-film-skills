...
四部，全部产自一张 **RTX 4070 Ti（12 GB）**，生成环节无云端 API。

下面是动图（无声）。**完整版带声音**：
▶ [狱卒](./docs/assets/yuzu.mp4) · [镜像·钟楼下](./docs/assets/jingxiang-zhonglou.mp4) ·
[镜像·天台](./docs/assets/jingxiang-tiantai.mp4) · [九尾](./docs/assets/jiuwei.mp4) ·
[山海](./docs/assets/shanhai.mp4)

### 《我为万界当狱卒》— long-shot 带对白的剧情片

<img src="./docs/assets/yuzu.gif" width="400">

▶ **[48 秒片段（有声）](./docs/assets/yuzu.mp4)** —— 第01–02集的 1'34"–2'22"

两集，40 镜 / 34 镜，各 1'47"。模型原生带口型对白，后期重配音色。
**这是第一部观众会来核对的片**：同一个人要在四十个镜头里还是同一个人，台词要接得上。

它推翻了我们自己的两条结论 —— 「提示词太长就削短」和「`audio: BGM通铺` 破不了」，
详见 [成片画廊](./docs/gallery.md)。

> ⚠️ **这条线到哪儿为止。** 又跑了两集之后我们停了：**12 GB 卡上这套做得出片子，
> 做不出短剧。** 736×416、每个 5 秒镜头约 7 分钟，68 镜是一整夜换 1'45" 的成片；
> 0.31 MP 里中景人脸只有 40–90 px，逼得带人物的镜里 76% 必须是近景以上 ——
> 而短剧要靠中景演对手戏。两个卡点都不是手艺问题，
> **也不是「换个简单画风」能绕过去的**（我们用的本来就是简单画风）。
> 下面所有方法论换到云端 API 或更大显存上照样成立，
> 完整说明见 [`dialogue-drama.md`](./skills/local-ai-film/knowledge/dialogue-drama.md)。

### 《镜像》— 上一条线走不通之后，改做「精片段」

<img src="./docs/assets/jingxiang-zhonglou.gif" width="400">

▶ **[钟楼下 33 秒（有声）](./docs/assets/jingxiang-zhonglou.mp4)** —— 9 镜 8 句台词，双女主正反打

<img src="./docs/assets/jingxiang-tiantai.gif" width="400">

▶ **[天台 14 秒（有声）](./docs/assets/jingxiang-tiantai.mp4)**

同一张卡、同一套 H3，**只把交付目标从「一部片」改成「几段站得住的 30 秒」**。
全片 58 段照跑（3 小时 21 分，失败 0），但不再当成品交 —— 从里面挑片段。

三条是这部片教的：

- **选片段的判据是景别，不是剧情。** 远景定场、全景双人这两档人小脸糊，
  一律不要；过肩和近景质量最好。**一段 30 秒可以零远景** ——
  少掉的信息由台词和声音补，比放一个糊掉的定场镜强。
- **片段不是从全片时间轴上切一段下来，是按片段自己的镜头表重剪。**
  全片里成立的正反打，切下来有一半在片段外面，读起来就成了「一张图在动嘴」。
- **第一版整档废，病因不是画风是没有角色参考图** —— 纯文生图出 47 张关键帧
  等于 47 次独立抽卡。第二版改成「发型形状 + 高饱和虹膜 + 一个不可能漂的硬记号」
  三件套锚，实拍零串装。详见 [`chain-consistency.md`](./skills/local-ai-film/knowledge/chain-consistency.md)。

⚠️ 这部片也是 `knowledge/grid-review.md`（格子法）和 `pitfalls.md` 坑 ⑲
（中间目录静默复用过期数据）的出处 —— 两条都是在它身上栽出来的。
...
| 声音 | BGM通铺 / 对白驱动 / 音效驱动 / 环境声 / 完全静音 / 声画错位 |
...
- **类比里的喻体会被画成实物** — 写「手腕上有块烫疤，**大小像一枚大硬币**」，手腕上真出现一个硬币状物件。比 `never X` 更难发现，因为你读那句话完全不觉得提到了硬币。
- **中间目录会静默复用过期数据** — 形状永远是「A 目录生成 B 目录、下游只读 B」，A 更新了 B 没重跑，谁也不报错。光后期链就四处（配音、超分、口型表、音效表）。最狠的一次是两天的配音工作一条都没进成片，**而且我连着两轮给出假绿灯 —— 因为我量的是 A，成片读的是 B**。修法两条：下游脚本第一行加 mtime 闸门；**验收必须从成片本身取证，量中间目录不算数**。
- **构图指令写了不执行，是权重之争不是措辞问题** — 三十个字的指令句被三百个字的画风锚和场景描述稀释掉，画面塌回模型默认构图。两条有效杠杆：**同一句指令在提示词首尾各放一遍**；**把构图写成物理上塞不下别的东西**（「上三分之一只有天、下三分之一只有地，他装在中间那条带子里」）。堆形容词**无效**。削长度有效，但那是**拿画质换服从** —— 背景要看的片子不划算。

链式长片（几十段接成一部）另有一整套坑，单独成篇在 `knowledge/chain-consistency.md`：
角色漂移要靠定期重锚定、锚定提示词必须带完整角色描述、空镜绝对不能锚定、
主角的「白发」会渗给同框的所有人。

**带对白的剧情片**又是一整套，在 `knowledge/dialogue-drama.md`：
角色与场景的共享常量层、模型做不到的六件事的**判停清单**（省得拿重抽去撞边界）、
台词的唯一来源、以及四个「每条时间轴各自都算得对、合起来打架」的后期 bug。

### 逐镜盖章：格子法（可选，默认不开）

`knowledge/grid-review.md`。一次要出几十个镜头的活，最容易犯的错是
**用「跑批器报没报错、文件在不在」代替「好不好」** —— 我们一天之内在关键帧定稿图、
关键帧候选图、视频段三个地方各栽一次；还有一批自动检查全绿、肉眼审片抓出 6 镜背影是错的。

做法是建一张由分镜表驱动的格子，每一格只有人眼看过画面并盖章才算数，
**盖章绑提示词指纹**（改了提示词章自动失效，否则两三轮之内就退化回「只看文件在不在」），
**全格盖满之前不进下游**（超分/剪辑/混音一跑，回头改一格成本翻十倍）。

> ⛔ **它默认不开，要先问用户。** 每一轮盖章都要把联络表当图片喂进上下文，
> 一部片下来是几十张图 —— **token 用量和上下文占用会明显上去**。
> 这是拿钱换「每一格都被看过一遍」，值不值得由用户决定，不由 Claude 替他决定。

还有一条是给盖章的人自己的：**拿不准的那一格要单独放大重截再判，
不要在联络表的缩图上定罪** —— 我们在缩图上误判过两次（把一个角色的后脑勺 + 裸肩
看成"一条大腿"），两次都差点去改一份根本没病的分镜。

### 无人值守跑批：把「我判不准」变成一个数

`knowledge/auto-review.md`。夜里没人可问，把存疑的攒到早上等于一夜 GPU 白烧。
于是每一个「这张能不能放行」都改成能算的：背对 → 人脸数 == 0；
人在画左 → 人体 bbox 中心 x；全景 → 膝/踝关键点 score。

**最容易跳过、也最要紧的一步是：检查器上岗前拿历史合格素材跑一遍回归，
查的是「误杀」不是「漏杀」。** 一个没验证过的检查器会整夜按错误判据杀好图，
比不装检查器坏得多。我们那次揪出三类自己的假阳性（方向词被当成位置词、
贴地拍鞋的镜头检出「8 个人」、过肩镜里量错了人），
顺带**独立查出 4 处已出片素材的真实缺陷**。
...
**有对白的片，第 6 步之前先读 `knowledge/dialogue-drama.md`** ——
常量层、判停清单、台词单一来源这几条要在写分镜前定死，写完再补是返工。
...
**按有没有对白**：没有对白的片可以整个绕开角色一致性（遮脸、逆光剪影，
我们有一部干脆 17 镜零露脸）；有对白的一样都绕不开，观众会核对。
这条分叉要读 `dialogue-drama.md`，在原生带音频的 long-shot 模型上还要配
`episode-drama` 骨架。
