...
### ⚠️ 泄漏的不只是文字：类比里的喻体会被画成实物

比文字泄漏更隐蔽的一层。写「手腕上有一块烫疤，**大小像一枚大硬币**」——
出来的图里手腕上真的有一个硬币状的物件。

**任何「像某某东西那么大」的写法都不能用**，比 `never X` 更难发现，
因为你自己读那句话完全不觉得提到了硬币。要写尺寸就写绝对量或占比。

### ⚠️ 描述痕迹要先定语气，不是先定形态

同一处旧疤连废三版，全在形态上打转（大小、位置、边缘），
其实错的是**语气**：`pink` / `glossy` 被读成「新伤」，
`faded` / `pale` / `smooth` 才是「旧疤」。

一般形式：**当你反复调形态却没有任何改善时，检查形容词的时态/新旧语气。**
...
⛔ **裁切必须放在超分之前**，这是最容易做反的一步。
出片之后再裁，等于在低分辨率上又裁一刀再放大，画面会明显变糊 ——
而本来就是 0.31MP 生成的，经不起这么折腾。
放在超分前，放大器会把那 1.16 倍一起补回来，几乎不掉画质。
（后期链的完整顺序见 `post-production.md`：剪辑 → **裁切** → 超分 → 调色 → 声音 → 字幕。）

⚠️ 有人会退而求其次去**逐帧转图片、擦掉字幕再合回去**。能work，但没必要 ——
只要裁切放对位置，这一步整个可以省掉。

### 需要口型的镜怎么办：台词躲不掉，改成「用它的口型、配自己的声音」

这一条和上面的规矩 1 直接冲突，所以单独说清楚：
**要口型就必须把台词写进提示词**，没有别的途径。做法是接受泄漏、再物理消除：

```
[Audio] "台词内容" — 角色名 (dialogue, lip-sync active, mouth clearly moving)
```

生成的音轨照样丢掉后期重配，写台词只是为了拿嘴型。字幕用上面的裁切解决。

⚠️ **口型命中率本身就不稳**：实测 12 条对白里 7 条嘴动良好（23%–73% 的帧），
5 条几乎不动（0%–7%），没有规律，只能跑完抽帧看、不合格的重跑。
所以**能用剪辑绕开的镜就别硬做口型** —— 台词比镜头长时，正确做法是
「说到一半切走」，切到反应镜，声音继续、画面已换（见 `dialogue-drama.md`）。
...
### .bat 用 LF 换行 → `cd /d` 静默失效

用 Python 生成跑批用的 `.bat` 时，如果按上面那条写了 `newline="\n"`，
就会踩到反面：**cmd.exe 读 LF 结尾的 bat 时，行尾会带进参数里**，
`cd /d "E:/…/镜像"` 当场失败，但 bat **不会停**，接着用**原来的工作目录**往下跑。

症状是 `python: can't open file 'E:\Projects\AI\kf.py'` —— 看起来像脚本丢了，
其实是没换成功目录。

```python
# 生成 .bat 一律 GBK + CRLF，和上面「写给 bash 的文件用 LF」正好相反
open(p, "w", encoding="gbk", newline="").write("\r\n".join(lines))
```

⭐ 判据：**这个文件将来由谁读**。bash / Python 读的用 LF+UTF-8，
cmd.exe 读的用 CRLF+GBK。别定一条全局规则。
...
角色卡里明明没写，出来的西装上却多了个蝴蝶结；同一个角色这一镜戴、下一镜不戴。
`fl2v` 和全能参考图都挡不住 —— 参考图管的是**长相和整体形制**，管不住这种局部空白。

**根因和坑 ⑧ 是同一条**：那个位置在画框里，而提示词里对它**没有任何描述**，
模型必须画点什么，就自由发挥。写「不要蝴蝶结」属于负向，一样无效。

### 修法：可选结构改成不可选

把那个位置**正面写死成唯一的样子**，别留空白：

| ✗ 留了空白 / 用负向 | ✅ 正面写死 |
|---|---|
| 「西装」 | 「西装，衬衫领口敞开两颗扣，颈前是裸露的皮肤，前襟平整没有任何装饰」 |
| 「西装，不要蝴蝶结」 | 同上 |
| 「短裙」 | 「裙筒是完整闭合的一圈、四周一样齐；裙摆离开腰之后是散开的、下缘悬空不贴腿」 |

⚠️ 那条短裙的例子是**两句都要写**的实例：只写前半句会出成短裤，
只写后半句会出成高衩连体衣 —— 空白填得不够死，模型照样有得挑。

### ⛔ 这类穿帮挑片救不回来，必须在写分镜阶段定死

可选配件（眼罩、领结、手套、发饰）**一旦在镜与镜之间闪烁，拼起来就是穿帮**：
观众看到的是同一个角色连续跳切，配件每隔一两刀变一次。

实测：某角色的眼罩在 20 刀里 7 戴 13 不戴，挑剩 6 刀之后戴/不戴正好 3:3，
**比例从 35% 涨到 50%，比原片更糟** —— 人在挑「哪一刀好看」，
不会顺带盯配件一致性。**只能在动作库/分镜阶段写死，不能指望后面补救。**

---

## ⑱ 旁白镜里，画面中的角色会跟着旁白动嘴 ★

旁白是画外音，画面里的人本该闭着嘴，实际却跟着一起张合。

### 两条一起用，缺一条都不够

1. **旁白镜显式声明说话人不在画面里**：
   `The narrator is never visible and no one on screen is speaking or moving their lips.`
2. **同框的每个角色单独标** `silent, mouth closed`。

只写第 1 句不够 —— 它约束的是"谁在说"，不是"画面里这个人的嘴"。

### 更确定的一招：改镜头设计，让嘴不在画面里

旁白镜干脆不给正脸：背影、过肩、极特写眼睛、手部。
嘴不进画框，就没有对不对得上的问题。

这和坑 ⑫「改主体不加负向」是同一个思路 ——
**能用确定性手段解决的，就别去赌种子**。旁白镜本来也不需要靠脸叙事。

---

## ⑲ 中间目录会静默复用过期数据，验收还量不到它 ★★

**适用：通用。这是我们踩过次数最多的一类，光后期链就四处。**

形状永远是同一个：

```
A 目录  ──某个脚本──▶  B 目录  ──下游只读──▶  成片
                        ↑
                  A 更新了，B 没重跑，谁也不报错
```
...
| 哪一处 | 症状 | 为什么查不出来 |
|---|---|---|
| **配音 `vo\ → vo_剪\`** | 改了两天的配音一条都没进成片 | 剪辑脚本里 `VO` 写死指向 `vo_剪\`，没有新旧检查 |
| **超分 `seg\ → seg_up\`** | 补拍过的段还是旧画面 | 跳过逻辑只比帧数，帧数一样就当已完成 |
| **口型表 `lip_onset.json`** | 补拍之后口型命中率全是假数 | 它是上一轮量出来的，补拍不会碰它 |
| **音效表（钉绝对秒）** | 剪辑一改，音效全落错位置 | 表里存的是秒不是镜号，剪辑变了它不知道 |

> **通则：凡是「A 目录生成 B 目录、下游只读 B」的地方，都要有一道 mtime 闸门。**
...
def 查新旧(src, dst):
    ...
    for f in sorted(os.listdir(src)):
        ...
        if not os.path.exists(b):
            旧.append(f + "(缺)")
        elif os.path.getmtime(b) < os.path.getmtime(a) - 1:      # 容差 1 秒
            旧.append(f)
    assert not 旧, "%s 里有 %d 条比 %s 旧或缺失，成片会用过期数据：%s\n" \
                   "  ⇒ 先重跑生成 %s 的那一步。" % (dst, len(旧), src, " ".join(旧[:8]), dst)
```

⛔ **闸门要放在下游脚本的第一行**，在它做任何事之前 —— 放在中间等于跑完一半才拦。

### ⛔⛔ 更贵的一半：验收落在了中间目录上

这个 bug 之所以能活两天，不是因为难查，是因为**我每次都量错了地方**：
我量 `vo\` 里的文件，说「8/8 对得上，没问题」，而成片读的是 `vo_剪\`。
**连着两轮假绿灯**，两轮都是我亲手给的。

> **验收必须落在用户真正打开的那个文件上。中间目录量得再准都不算数。**

修法是把验证脚本改成从**成片本身**取证 —— 按字幕时间窗从成片音轨里切出每一句，
分别和「应该是的那一版」「被换掉的那一版」做互相关，看哪个赢：

```
镜    台词                  应该是的   旧件    判
E01  地铁站那次，是有人栽赃。   0.552    0.119   ✅
F02  是同一个人？            0.960    0.376   ✅
```

（成片里混了 BGM，所以不会到 1.0，比的是**谁明显更高**。）

同类：坑 ② 的「判定成功要看产物不要看日志」、坑 ⑩ 的「验收必须抽帧」。
**三条是同一条在不同层的样子：证据只能取自终点。**
