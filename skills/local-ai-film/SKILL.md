...
| `knowledge/` | 踩坑经验：工程坑、提示词、分镜表、链式一致性、**对白剧情片**、**逐镜验收（格子法）**、**无人值守验收**、后期、声音 |
...
- **时间轴按磁盘实测**，不能用计划值推算
- **每一处「A 目录生成 B 目录、下游只读 B」都要有 mtime 闸门**，否则成片会静默用过期数据；
  而且**验收要从成片本身取证，不能量中间目录**（坑 ⑲ —— 我们在这上面连给两轮假绿灯）
...
**⛔ 默认不开，而且要先问用户。** 它有效，但代价是**用户的 token 和上下文**：
每一轮盖章都要把联络表当图片喂进上下文，一部片下来是几十张图。
...
做法读 `knowledge/grid-review.md`。一句话版本：
**建一张由分镜表驱动的格子，每一格只有我自己看过画面并盖章才算数，
盖章绑提示词指纹（改了提示词章自动失效），全格盖满之前不进下游。**

⛔ 里面有一条别跳过：**拿不准的那一格要单独放大重截再判，不要在联络表的缩图上定罪。**
我们在缩图上误判过两次，两次都差点去改一份根本没病的分镜。
